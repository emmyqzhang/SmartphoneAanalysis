...
public class RetweetBeen {
	...
	// public TweetBeen(Status status, String manufacturer, String smartphone) {
	// this(status.getId(), status.getCreatedAt(),
	// status.getCurrentUserRetweetId(), status.getFavoriteCount(),
	// status.getInReplyToScreenName(), status.getInReplyToStatusId(),
	// status.getInReplyToUserId(),
	// status.getLang(), status.getText(), status.getUser().getScreenName(),
	// (status.isTruncated() ? "1" : "0"), status.getPlace().getCountry(),
	// status.getPlace().getCountryCode(),
	// manufacturer, smartphone);
	// }

	public RetweetBeen(long id, long tweet_id, Date createdAt, long currentUserRetweetId, int favoriteCount,
